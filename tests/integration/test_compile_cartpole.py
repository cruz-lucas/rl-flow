...
def test_compile_builtin_dqn_navix_fixture(tmp_path: Path) -> None:
    workflow = WorkflowSpec.model_validate(
        yaml.safe_load(
            Path("configs/workflows/navix_dqn_empty_room.yaml").read_text(encoding="utf-8")
        )
    )
    ...
    assert "rlflow_builtin.runners.tabular_jax" in (tmp_path / "command.sh").read_text(
        encoding="utf-8"
    )
    ...
    assert (
        yaml.safe_load((tmp_path / "status.json").read_text(encoding="utf-8"))["status"]
        == "completed"
    )
